...
float prev_sample_sump = 0,prev_sample_ot = 0;
float max_ot_depth = 125.0;//in cm
float max_sump_depth = 150.0;//in cm

float Water_Distance(char TRIGPIN,char ECHOPIN) {
...
void Alive_Counter()
{
  ...
  if (Firebase.ready() && signupOK ) 
    {
      if (Firebase.RTDB.setFloat(&fbdo, "IOT_Control_4Load_mod/AC",alive_counter))
      {
...
float Sampler(char TRIGPIN,char ECHOPIN, float max_depth,float prev_sample,String S_Upload)
{
  //Start
  int i = 0;
  String S_error=""; 
  String S_final = "IOT_Control_4Load_mod/" + S_Upload;
  float water_percentage = 0;
  float result = 0;
  float duration = 0, currrent_sample = 0;
  ...
  // Use 343 metres per second as speed of sound
  // Divide by 1000 as we want millimeters
  currrent_sample = (duration / 2) * 0.0343;
  ...
  if(result < 10)
  {
    water_percentage = (currrent_sample * (max_depth/100.0));
    ...
    if (Firebase.ready() && signupOK ) 
    {
      if (Firebase.RTDB.setFloat(&fbdo, S_final,water_percentage))
      {
  ...
  else
  {
    if (Firebase.ready() && signupOK ) 
    {
      if (Firebase.RTDB.setFloat(&fbdo, S_final,255))
      {
        Serial.print("Water Percentage: ");
        Serial.println(255);
      }
...
void loop()
{
  ...
  prev_sample_sump = Sampler(TRIGPIN_SUMP,ECHOPIN_SUMP,max_sump_depth,prev_sample_sump,"WP");
  prev_sample_ot = Sampler(TRIGPIN_OT,ECHOPIN_OT,max_ot_depth,prev_sample_ot,"OP");
  Alive_Counter();
