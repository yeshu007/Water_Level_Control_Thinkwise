...
void Sampler_OT()
{
  ...
  if(currrent_sample_ot > 20) 
  {
    currrent_sample_ot = currrent_sample_ot - 20; //dist in cm from USS to OF sensor
  }
  else
  {
  ...
  if(result < 10)
  {
    ...
    if (Firebase.ready() && signupOK ) 
    {
      if (Firebase.RTDB.setFloat(&fbdo, "IOT_Control_4Load_mod/OP",water_percentage))
      {
  ...
  else
  {
    if (Firebase.ready() && signupOK ) 
    {
      if (Firebase.RTDB.setFloat(&fbdo, "IOT_Control_4Load_mod/OP",255))
      {
        Serial.print("Water Percentage_OT: ");
        Serial.println(255);
      }
