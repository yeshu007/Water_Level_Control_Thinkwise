...
#ifndef STASSID
#define STASSID "Jio-Yesh"
#define STAPSK "Hanu@123"
#endif
...
// Function to load the configuration data from LittleFS 
bool loadConfig(){  
...
void readFilemod(const char *path) {
  char ch[200] = {'\0'};
  int i=0;
...
void setup() {
  ...
  Serial.println("listing DIR 1");
  listDir("/");
  //deleteFile("/hello.txt");
  writeFile("/config.txt", "SSID: ");
  appendFile("/config.txt", "PWD: ");
  appendFile("/config.txt", "EOF\n");

  writeFile("/config1.txt", "");
  writeFile("/config2.txt", "");

  readFile("/config.txt");
  Serial.println("listing DIR 2");
  listDir("/");
  Serial.println("Reading config1.txt");
  readFile("/config1.txt");
  Serial.println("Reading config2.txt");
  readFile("/config2.txt");
